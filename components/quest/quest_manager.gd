...
func accept_quest(quest_id: String) -> bool:
	...
	if not quest:
		return false
	return GameManager.add_quest_by_id(quest_id, true)
...
const _GM_HANDLED_ITEM_IDS: Array[String] = [
	"fugleskinn", "elgskinn", "peak_performance_lua", "gard_plakat", 
	"kobber", "plast", "gummi", 
]


func _on_game_manager_item_added(item_id: String, amount: int) -> void :
	if item_id in _GM_HANDLED_ITEM_IDS:
		return
