...
const PROMPT_OPEN: = "E for å åpne stipendskjema"
...
func _ready() -> void :
	if label_3d:
		label_3d.text = PROMPT_OPEN
		label_3d.hide()

func _process(_delta: float) -> void :
	if not in_range or label_3d == null:
		return
	var wait_left: float = GameManager.get_scholarship_penalty_remaining()
	if wait_left > 0.0:
		label_3d.text = "Prøv igjen om %ds" % int(ceil(wait_left))
		label_3d.show()
	else:
		label_3d.text = PROMPT_OPEN
...
func _input(_event: InputEvent) -> void :
	...
	if DialogueUI.is_open():
		return
	if GameManager.get_scholarship_penalty_remaining() > 0.0:
		return
