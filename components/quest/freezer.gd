...
func _spawn_ice_creams():
	for i in range(spawn_points.size()):
		...
		# Deterministic name (not the engine's auto-numbered default) so this
		# node's path is identical on every peer — a prerequisite for the
		# carry sync PlayerCharacterScript sets up on pickup (see
		# multiplayer/carriable_net.gd) to find "the same" node across peers.
		ice_cream.name = "%s_IceCream_%d" % [name, i]
