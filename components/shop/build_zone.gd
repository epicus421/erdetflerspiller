...
@export var required_scraps: int = 4
...
func _try_activate() -> void :
	if _stand_spawned or _activated:
		return
	if GameManager == null or not GameManager.inheritance_spent_on_non_ice_cream:
		return
	_activated = true
	_spawn_preview()
...
func _can_build() -> bool:
	return (
		GameManager != null
		and GameManager.inheritance_spent_on_non_ice_cream
		and _activated
		and _player_in_range
		and _scraps_in_zone.size() >= required_scraps
		and not _stand_spawned
	)
