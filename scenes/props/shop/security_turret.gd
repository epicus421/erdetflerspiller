...
func _ready() -> void :
	if alarm_light:
		alarm_light.remove_from_group("SceneLights")
...
func _track_player(delta: float) -> void :
	if swivel == null:
		return
	var player: = NetUtil.get_nearest_player(swivel.global_position)
	if player == null:
		return
	...
	if forward.length_squared() < 1e-08:
		return
...
func _fire() -> void :
	...
	if health and health.has_method("take_damage"):
		GameManager.last_death_cause = "security_turret"
